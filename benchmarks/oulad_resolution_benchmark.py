...
def build_table(info,sa,ass,sv,cutoff=120):
    ...
    for c in ["assess_mean","assess_n","submission_mean","vle_clicks","active_days","vle_records"]: d[c]=d[c].fillna(0)
    return d

def qbin(s,q=5): return pd.qcut(s.rank(method="first"),q,labels=False,duplicates="drop").astype(int)

def unresolved_rate(d,cols,q=5):
    x=d.copy(); keys=[]
    for c in cols:
        k=c+"_bin"; x[k]=qbin(x[c],q); keys.append(k)
    g=x.groupby(keys)["certify"].agg(["min","max","size"]).reset_index(); bad=g[g["min"]!=g["max"]]
    n=int(bad["size"].sum()); return n/len(x),n,len(bad)

def evaluate(cutoff=120,q=5):
    d=build_table(*load(),cutoff=cutoff); base=["assess_mean"]
    candidates={"assessment_count":["assess_n"],"vle_engagement":["vle_clicks"],"active_days":["active_days"],"submission_timing":["submission_mean"],"vle_records":["vle_records"]}
    r0,_,_=unresolved_rate(d,base,q); rows=[]
    for name,extra in candidates.items():
        r,n,c=unresolved_rate(d,base+extra,q)
        rows.append({"experiment":name,"n_student_modules":len(d),"cutoff":cutoff,"q":q,"base_unresolved_rate":r0,"post_unresolved_rate":r,"absolute_resolution_gain":r0-r,"remaining_unresolved_students":n,"mixed_cells":c,"declared_cost":1.0,"gain_per_cost":r0-r})
